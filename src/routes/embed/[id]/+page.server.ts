...
import { GhostApiClient, GhostApiClientCredentials } from '$lib/api/ghost.js';
...
export const load: PageServerLoad = async ({ url, params }) => {
	try {
		...
		// Get Ghost API credentials from params or environment
        const id = params.id || urlParams.get('id')
		const ghostAdminApiUrl = id ? GhostApiClientCredentials.getCredentials()[id].apiUrl : env.GHOST_ADMIN_API_URL;
		const ghostAdminApiKey = id ? GhostApiClientCredentials.getCredentials()[id].apiKey : env.GHOST_ADMIN_API_KEY;
